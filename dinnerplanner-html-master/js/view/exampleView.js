...
var ExampleView = function (container, model) {
	...
	this.addDish = function(id) {
		...
		this.selectedDish.replaceWith(model.getSpecificDish(id));

		this.specPrice = container.find("#selectedPrice");
		this.specPrice.replaceWith(model.getDishPrice(id));
		$("#addedDishes").append('<div class="row addDishesRow" style="background-color: #D8D8D8; color: #B40404; border: 2px solid black; margin-top: 10px; margin-left: 1px; margin-right: 1px;">'
			...
			+'<button type="button" class="btn btn-default btn-xs remove" id='+id+' style="background-color: #B40404; border-radius: 15px; margin-left: 175px; margin-top: -20px;"><span class="glyphicon glyphicon-remove"></span></button></div>');
